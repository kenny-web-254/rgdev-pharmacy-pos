...
path = Path('src/components/ReceiptSettingsView.tsx')
s = path.read_text(encoding='utf-8') if path.exists() else ''

# Recover the receipt settings screen if an earlier automated cleanup accidentally
# left only its import header. Keep this production fallback intentionally small
# and dependency-free so the deployment cannot be blocked by legacy mock data.
if len(s) > 2000 and "../data/mockData" not in s:
    print('Receipt settings component already intact.')
else:
  ...
  return <div className="space-y-6">
    ...
    {tab === 'database' ? <SupabaseDatabaseSettings isAdmin={admin} /> : <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-5">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <label className="text-xs font-semibold text-slate-700">Tax Rate<input disabled={!admin} type="number" min="0" max="1" step="0.01" value={form.taxRate} onChange={(e) => update('taxRate', Number(e.target.value) || 0)} className="mt-1 w-full px-3 py-2 rounded-xl border border-slate-300 text-sm" /></label>
      </div>
...
''', encoding='utf-8')
    print('Recovered production receipt settings component.')
