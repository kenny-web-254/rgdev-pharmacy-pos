...
type PreviewRow = { rowNumber: number; medication: Medication; action: 'CREATE' | 'UPDATE'; stockAction: ImportAction; errors: string[] };
...
function buildMedication(row: ImportRow, existing?: Medication): { medication: Medication; errors: string[]; stockAction: ImportAction } {
  const errors: string[] = [];
  ...
  const barcode = text(row, 'Barcode / NDC', 'Barcode', 'NDC') || existing?.barcode || '';
  ...
  const handleFile = async (file?: File) => {
    ...
    try {
      ...
      const result: PreviewRow[] = rows.map((row, index) => {
        const barcode = text(row, 'Barcode / NDC', 'Barcode', 'NDC').toLowerCase();
        ...
        return { rowNumber: index + 2, medication: built.medication, action: existing ? 'UPDATE' : 'CREATE', stockAction: built.stockAction, errors };
      });
      setPreview(result);
    } catch (e) {
