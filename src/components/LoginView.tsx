...
import { AlertCircle, Clock, Eye, EyeOff, KeyRound, Lock, LogIn, ShieldCheck, User as UserIcon } from 'lucide-react';
import { User } from '../types';
import { signInWithSupabase, supabaseConfig } from '../services/supabase';
...
export const LoginView: React.FC<LoginViewProps> = ({
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    if (!supabaseConfig.isConfigured()) {
      setError('Supabase is not configured. Configure VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in the deployment environment.');
      return;
    }
      ...
      <div className="w-full max-w-md relative z-10 animate-fade-in">
        <div className="text-center mb-6">
          <div className="w-14 h-14 rounded-2xl bg-teal-600 text-white flex items-center justify-center mx-auto shadow-lg shadow-teal-500/20 mb-3 border border-teal-400/30">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8 text-teal-100">
          ...
          <p className="text-xs text-teal-300/80 font-medium mt-1">Pharmacy & Clinical Point-of-Sale System</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 p-6 sm:p-8 space-y-5">
          ...
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-500">Sign in with an authorized Supabase Auth account.</p>
            </div>
          ...
          <form onSubmit={handleSignIn} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" required autoFocus value={identifier} onChange={(e) => setIdentifier(e.target.value)} placeholder="user@pharmacy.com or username" className="w-full pl-10 pr-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-900 focus:bg-white focus:outline-hidden focus:ring-2 focus:ring-teal-600 focus:border-transparent transition" />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPassword((v) => !v)} className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-slate-400 hover:text-slate-600 transition" tabIndex={-1}>
                  {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                </button>
              </div>
            </div>

            <button type="submit" disabled={isSubmitting} className="w-full py-3 px-4 bg-teal-700 hover:bg-teal-800 active:scale-[0.98] text-white rounded-xl text-xs font-bold shadow-sm transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60">
          ...
          <div className="pt-2 border-t border-slate-100 text-[11px] text-slate-500 space-y-1.5">
            ...
            <p className="text-[11px] leading-relaxed text-slate-400">
              New accounts are provisioned by an Administrator. Self-registration is disabled.
            </p>
          </div>
        </div>
      </div>
