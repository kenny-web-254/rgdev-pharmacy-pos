...
function rowToClinicalTest(r: any): ClinicalTest {
  return {
    ...
    referenceRanges: r.reference_ranges || undefined,
    notes: r.notes || undefined,
    requestedBy: r.requested_by,
    conductedAt: r.conducted_at || undefined,
    createdAt: r.created_at,
  };
}
...
export async function fetchPatientsFromSupabase(): Promise<Patient[] | null> {
  const c = getSupabase();
  if (!c) return null;
  const { data, error } = await c.from('patients').select('*').order('full_name', { ascending: true });
  if (error) { console.error('Failed to load patients from Supabase', error.message); return null; }
  return (data || []).map(rowToPatient);
}

export async function fetchConsultationsFromSupabase(): Promise<Consultation[] | null> {
  const c = getSupabase();
  if (!c) return null;
  const { data, error } = await c.from('consultations').select('*').order('date', { ascending: false });
  if (error) { console.error('Failed to load consultations from Supabase', error.message); return null; }
  return (data || []).map(rowToConsultation);
}

export async function fetchClinicalTestsFromSupabase(): Promise<ClinicalTest[] | null> {
  const c = getSupabase();
  if (!c) return null;
  const { data, error } = await c.from('clinical_tests').select('*').order('created_at', { ascending: false });
  if (error) { console.error('Failed to load clinical tests from Supabase', error.message); return null; }
  return (data || []).map(rowToClinicalTest);
}

function rowToSale(row:any):SaleTransaction{return{id:row.id,receiptNumber:row.receipt_number,timestamp:row.timestamp,cashierName:row.cashier_name,cashierRole:row.cashier_role as UserRole,items:Array.isArray(row.items)?row.items:[],subtotal:Number(row.subtotal||0),tax:Number(row.tax||0),discount:Number(row.discount||0),total:Number(row.total||0),paymentMethod:row.payment_method,amountTendered:row.amount_tendered==null?undefined:Number(row.amount_tendered),changeDue:row.change_due==null?undefined:Number(row.change_due),cashAmount:row.cash_amount==null?undefined:Number(row.cash_amount),mpesaAmount:row.mpesa_amount==null?undefined:Number(row.mpesa_amount),mpesaReference:row.mpesa_reference||undefined,mpesaPhone:row.mpesa_phone||undefined,patientName:row.patient_name||undefined,cardAuthCode:row.card_auth_code||undefined,insuranceProvider:row.insurance_provider||undefined,insurancePolicyNumber:row.insurance_policy_number||undefined,insuranceAuthCode:row.insurance_auth_code||undefined,isOffline:Boolean(row.is_offline),synced:Boolean(row.synced),syncTimestamp:row.sync_timestamp||undefined};}
...
export async function pullReceiptSettingsFromSupabase():Promise<ReceiptSettings|null>{const c=getSupabase();if(!c)return null;const{data,error}=await c.from('receipt_settings').select('*').limit(1).maybeSingle();if(error||!data)return null;return{pharmacyName:data.pharmacy_name||'RG Pharma-POS',tagline:data.tagline||'',addressLine1:data.address_line1||'',addressLine2:data.address_line2||'',phone:data.phone||'',email:data.email||'',licenseNumber:data.license_number||'',taxId:data.tax_id||'',taxRate:Number(data.tax_rate||0),paperWidth:data.paper_width==='58mm'?'58mm':'80mm',headerMessage:data.header_message||'',footerMessage:data.footer_message||'',returnPolicy:data.return_policy||'',emergencyPhone:data.emergency_phone||'',showGenericName:Boolean(data.show_generic_name),showBatchAndExpiry:Boolean(data.show_batch_and_expiry),showPrescriptionDetails:Boolean(data.show_prescription_details),showPharmacistName:Boolean(data.show_pharmacist_name),showBarcode:Boolean(data.show_barcode),showTaxBreakdown:Boolean(data.show_tax_breakdown),currencySymbol:data.currency_symbol||'KSh'};}

// Self-service profile/password changes. Distinct from the admin-only
// createManagedUser/updateManagedUser/etc (which go through the
// admin-user-provisioning Edge Function): RLS already allows a user to
// update their own pharmacy_users row, and changing one's own password is
// a normal Supabase Auth operation that needs no elevated privilege.
export async function updateOwnProfile(updates: { name?: string; email?: string; phone?: string; licenseNumber?: string }): Promise<{ ok: boolean; error?: string }> {
  const c = getSupabase();
  if (!c) return { ok: false, error: 'Supabase is not configured.' };
  const { data: sessionData } = await c.auth.getSession();
  const authUserId = sessionData.session?.user?.id;
  if (!authUserId) return { ok: false, error: 'Your session has expired. Please sign in again.' };
  const row: Record<string, unknown> = {};
  if (updates.name !== undefined) row.name = updates.name;
  if (updates.email !== undefined) row.email = updates.email;
  if (updates.phone !== undefined) row.phone = updates.phone;
  if (updates.licenseNumber !== undefined) row.license_number = updates.licenseNumber;
  const { error } = await c.from('pharmacy_users').update(row).eq('auth_user_id', authUserId);
  if (error) return { ok: false, error: error.message };
  return { ok: true };
}

export async function changeOwnPassword(email: string, currentPassword: string, newPassword: string): Promise<{ ok: boolean; error?: string }> {
  const c = getSupabase();
  if (!c) return { ok: false, error: 'Supabase is not configured.' };
  const { error: verifyError } = await c.auth.signInWithPassword({ email, password: currentPassword });
  if (verifyError) return { ok: false, error: 'Current password entered is incorrect.' };
  const { error: updateError } = await c.auth.updateUser({ password: newPassword });
