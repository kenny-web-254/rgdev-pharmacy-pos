...
function rowToUser(row:any):User{const rawRole=String(row.role||'').trim().toLowerCase();const role=(rawRole==='admin'||rawRole==='clinician'||rawRole==='cashier'?rawRole:'cashier') as UserRole;const rawStatus=String(row.status||'active').trim().toLowerCase();return{id:row.id,username:row.username,email:row.email||undefined,name:row.name,role,status:rawStatus==='inactive'?'inactive':'active',createdAt:row.created_at,lastLogin:row.last_login||undefined,phone:row.phone||undefined,licenseNumber:row.license_number||undefined,avatarColor:row.avatar_color||'bg-teal-700'};}
