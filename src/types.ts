...
export interface ClinicalTest { id: string; consultationId?: string; patientId: string; patientName?: string; patientDOB?: string; patientPhone?: string; testNumber?: string; testName: string; testType?: string; category: 'Hematology' | 'Biochemistry' | 'Microbiology' | 'Rapid Diagnostic' | 'Urinalysis' | 'Other'; status: 'Pending' | 'In Progress' | 'Completed' | 'Cancelled'; results?: string; resultSummary?: string; referenceRanges?: string; notes?: string; requestedBy: string; clinicianName?: string; clinicianLicense?: string; dateOrdered?: string; resultDate?: string; linkedPrescriptionId?: string; conductedAt?: string; createdAt: string; }
