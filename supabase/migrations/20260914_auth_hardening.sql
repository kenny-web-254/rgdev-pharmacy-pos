...
SET role=lower(role), auth_user_id='70e222d9-604f-4b32-a477-b31ab6e39cc4', updated_at=now()
WHERE id='70e222d9-604f-4b32-a477-b31ab6e39cc4' AND email='admin@rgdev.pos';
...
AS $$ SELECT role FROM public.pharmacy_users WHERE auth_user_id=(SELECT auth.uid()) AND status='active' LIMIT 1; $$;
...
DROP FUNCTION IF EXISTS public.current_pharmacy_role();
...
CREATE POLICY pharmacy_users_select ON public.pharmacy_users FOR SELECT TO authenticated
USING (auth_user_id=(SELECT auth.uid()) OR (SELECT private.current_pharmacy_role())='admin');
CREATE POLICY pharmacy_users_insert ON public.pharmacy_users FOR INSERT TO authenticated
WITH CHECK ((SELECT private.current_pharmacy_role())='admin');
CREATE POLICY pharmacy_users_update ON public.pharmacy_users FOR UPDATE TO authenticated
USING (auth_user_id=(SELECT auth.uid()) OR (SELECT private.current_pharmacy_role())='admin')
WITH CHECK (auth_user_id=(SELECT auth.uid()) OR (SELECT private.current_pharmacy_role())='admin');
CREATE POLICY pharmacy_users_delete ON public.pharmacy_users FOR DELETE TO authenticated
