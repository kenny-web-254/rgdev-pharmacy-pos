-- Inventory unit/pricing hardening and authoritative Excel import.
-- Safe for existing rows: all new columns already have defaults.
...
as $$
declare
  v_role text; v_row jsonb; v_barcode text; v_name text; v_generic text; v_dosage text;
  v_form text; v_category text; v_batch text; v_expiry date; v_manufacturer text;
  v_price numeric; v_cost numeric; v_qty integer; v_min_stock integer; v_action text;
  v_rx boolean; v_cold boolean; v_id text; v_existing public.medications; v_new_stock integer;
  v_created integer := 0; v_updated integer := 0; v_index integer := 0;
  v_user_id text; v_user_name text; v_user_role text;
  v_pack_size numeric; v_stock_unit text; v_sale_unit text; v_can_sell_individually boolean;
  v_unit_price numeric; v_unit_cost numeric;
begin
  v_role := private.current_pharmacy_role();
  if v_role is null or v_role <> 'admin' then raise exception 'Only administrators may import inventory'; end if;
  ...
  for v_row in select value from jsonb_array_elements(p_rows) loop
    v_index := v_index + 1;
    v_id := nullif(trim(v_row->>'id'), ''); v_name := nullif(trim(v_row->>'name'), '');
    v_generic := nullif(trim(v_row->>'genericName'), ''); v_dosage := nullif(trim(v_row->>'dosage'), '');
    v_form := coalesce(nullif(trim(v_row->>'form'), ''), 'Unit');
    v_category := coalesce(nullif(trim(v_row->>'category'), ''), 'Uncategorized');
    v_barcode := nullif(trim(v_row->>'barcode'), ''); v_batch := nullif(trim(v_row->>'batchNumber'), '');
    v_manufacturer := coalesce(trim(v_row->>'manufacturer'), '');
    v_action := upper(coalesce(nullif(trim(v_row->>'stockAction'), ''), 'ADD'));
    v_price := nullif(v_row->>'price','')::numeric; v_cost := nullif(v_row->>'costPrice','')::numeric;
    ...
    if v_can_sell_individually and v_pack_size <= 1 then raise exception 'Subunit tracking requires a pack size greater than 1 at import row %', v_index; end if;
    if v_can_sell_individually and v_unit_price is null then raise exception 'Smallest-unit price is required when subunit tracking is enabled at import row %', v_index; end if;
    ...
    if v_existing.id is null then
      ...
      values(v_id,v_name,v_generic,v_dosage,v_form,v_category,v_rx,v_barcode,ceil(v_price),v_cost,v_qty,v_min_stock,v_batch,v_expiry,v_manufacturer,v_cold,v_pack_size,v_stock_unit,v_sale_unit,v_can_sell_individually,coalesce(v_unit_price, v_price / v_pack_size),coalesce(v_unit_cost, v_cost / v_pack_size),now(),now());
      v_created := v_created + 1;
    else
      if v_action = 'ADD' then v_new_stock := v_existing.stock + v_qty;
      elsif v_action = 'REDUCE' then v_new_stock := v_existing.stock - v_qty; if v_new_stock < 0 then raise exception 'REDUCE would make stock negative at import row %', v_index; end if;
      else v_new_stock := v_qty; end if;
      update public.medications set name=v_name,generic_name=v_generic,dosage=v_dosage,form=v_form,category=v_category,is_prescription_required=v_rx,price=ceil(v_price),cost_price=v_cost,stock=v_new_stock,min_stock_level=v_min_stock,batch_number=v_batch,expiry_date=v_expiry,manufacturer=v_manufacturer,requires_refrigeration=v_cold,pack_size=v_pack_size,stock_unit=v_stock_unit,sale_unit=v_sale_unit,can_sell_individually=v_can_sell_individually,unit_price=coalesce(v_unit_price,v_price/v_pack_size),unit_cost=coalesce(v_unit_cost,v_cost/v_pack_size),updated_at=now() where id=v_existing.id;
...
revoke all on function public.import_inventory(jsonb) from public, anon;
grant execute on function public.import_inventory(jsonb) to authenticated;

alter table public.medications drop constraint if exists medications_pack_size_check;
alter table public.medications add constraint medications_pack_size_check check (pack_size > 0);
alter table public.medications drop constraint if exists medications_unit_price_check;
alter table public.medications add constraint medications_unit_price_check check (unit_price is null or unit_price >= 0);
alter table public.medications drop constraint if exists medications_unit_cost_check;
alter table public.medications add constraint medications_unit_cost_check check (unit_cost is null or unit_cost >= 0);
alter table public.medications drop constraint if exists medications_units_not_blank_check;
alter table public.medications add constraint medications_units_not_blank_check check (length(trim(stock_unit)) > 0 and length(trim(sale_unit)) > 0);
